...
/**
 * ConsultingServicesKycPage component renders the KYC (Know Your Customer) form 
 * specifically tailored for users providing Consulting Services.
 * It collects professional subdivision, basic, and professional information as defined in KYC.md.
 */
export default function ConsultingServicesKycPage() {
    ...
    <div className="container mx-auto p-8 font-sans bg-gray-50 min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-xl space-y-8">
        ...
        <section>
          ...
          <FormField label="主要顾问类型" id="consultantType" placeholder="例如: 商业顾问, 管理顾问, 技术顾问 (可多选)" />
          <FormField label="其他顾问类型 (请注明)" id="consultantTypeOther" placeholder="若上方未包含您的类型" />
        </section>
        ...
        <section>
          ...
          <FormField label="服务范围或咨询方向" id="serviceScope" isTextarea placeholder="例如: 市场策略, 流程优化, IT架构咨询等" />
          <FormField label="案例经验或项目经验" id="caseExperience" isTextarea placeholder="请列举主要参与或负责的咨询项目" />
          <FormField label="专业技能或技术领域" id="technicalSkills" isTextarea placeholder="您的核心专业技能和技术栈" />
          <FormField label="使用工具或方法论" id="toolsMethodology" isTextarea placeholder="例如: SWOT分析, 敏捷管理, 特定行业软件等" />
          <FormField label="技能证书或行业认证 (可选)" id="certifications" isTextarea placeholder="请列举相关证书或认证" />
