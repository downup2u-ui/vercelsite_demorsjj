...
/**
 * CraftsmanKycPage component renders the KYC (Know Your Customer) form 
 * specifically tailored for users registering as Craftsmen.
 * It collects basic, extended, and professional information as defined in KYC.md.
 */
export default function CraftsmanKycPage() {
    ...
    <div className="container mx-auto p-8 font-sans bg-gray-50 min-h-screen">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-xl space-y-8">
        ...
        <section>
          ...
          <FormField label="主要手工艺类别" id="craftCategory" placeholder="例如: 珠宝首饰, 混合媒介, 陶瓷, 木工, 编织 (可多选)" />
          <FormField label="其他手工艺类别 (请注明)" id="craftCategoryOther" placeholder="若上方未包含您的类别" />
        </section>

        <section>
          <h2 className="text-xl font-semibold text-gray-700 mb-6 border-b pb-2">基本信息</h2>
          <FormField label="姓名 / 昵称" id="name" placeholder="您的姓名或店铺名称" />
        ...
        <section>
          ...
          <FormField label="擅长材料或风格" id="materialsStyle" isTextarea placeholder="例如: 纯银, 天然宝石, 简约风格, 民族风" />
          <FormField label="作品集链接" id="portfolioLink" type="url" placeholder="例如: Etsy 店铺, 个人网站, Instagram 作品展示" />
          <FormField label="创作方向" id="creativeDirection" isTextarea placeholder="您的主要创作理念或主题" />
        </section>

        <section>
          <h2 className="text-xl font-semibold text-gray-700 mb-6 border-b pb-2">专业信息</h2>
          <FormField label="主要使用工具或工艺" id="toolsProcess" isTextarea placeholder="例如: 雕金工艺, 手工编织机, 陶艺拉坯机" />
          <FormField label="技能证书或获奖经历 (可选)" id="awards" isTextarea placeholder="请列举重要的证书或获奖情况" />
